...
miLLAVE='W1dJl8FYPOlhejAfYrg9Am7zbMEFYsRBZ0IgsZtC'
GUID="http://api.datosabiertos.msi.gob.pe/api/v2/datastreams/ORDEN-DE-BIENE-SERVI-2020/"
FORMATO='data.json/'

#Visualisamos el request

request=paste0(GUID,FORMATO,'?auth_key=',miLLAVE)
request

ordenSI = fromJSON(request)
ordenSI = ordenSI[["result"]][["fArray"]]
#cambiando el formato
FORMATO='data.pjson/'
request2=paste0(GUID,FORMATO,'?auth_key=',miLLAVE)
request2
ordenSI2 = fromJSON(request2)

ordenSI2 = ordenSI2[["result"]]
...
data$descripcion = str_to_lower(data$`Descripcion-de-la-Orden`)
#otra opción
...
```{r}
#Otro método para sacar el API
millave2 = 'V0REXgLBkKffVgEPId60k0FHHrgXanFDwpXfDQWm'
...
head(candidatos$nombre)
head(candidatos$apellido)
```
...
Vemos que la variable de nuestro interés "GDP - per capita (PPP)" está sucia pues tiene el símbolo "$" y ",".
...
```{r}
names(data2)
data2[1,3] #Con esto podemos ver el primer elemento de ese vector para darle una revisión
data2$var1 = parse_number(data2$Exports)
head(data2$var1)
str(data2)
```

Luego lo cambiamos de escala (en millones)
...
data3$numeros = parse_number(data3$var1)

data3$cantidad = str_split(string = data3$var1,
                         pattern = ' ',
                         simplify = T)[,2]
```
...
```{r}
table(data3$cantidad)
data3$cantidad = gsub('million',"1000000",data3$cantidad) # reemplaza la palabra million por el número correspondiente.
```

La palabra "billion" por 1000000000:

```{r}
data3$cantidad = gsub('billion',"1000000000",data3$cantidad) # reemplaza la palabra billion por el número correspondiente.
```
...
```{r}
#otra opcion
data3$cantidad = gsub('Mt\\(2010',"1",data3$cantidad)
data3$cantidad = gsub('Mt\\(2012',"1",data3$cantidad)
data3$cantidad = gsub('Mt\\(2017',"1",data3$cantidad)
...
```{r}
str(data3$cantidad)
data3$cantidad = as.numeric(data3$cantidad)
```
...
```{r}
data3$final = (data3$numeros*data3$cantidad)
data3$finalbilones = (data3$final/1000000000)
```
